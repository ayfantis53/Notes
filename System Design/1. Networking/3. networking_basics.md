...
- **Wide-Area-Network**: (WAN) Connects devices over a large geographical area, potentially across continents. The internet is the largest example. 
...
#### **Repeater**: receives a signal, amplifies it, and retransmits it to extend the range of a network.
...
#### **Hub**: a basic device that connects multiple computers or devices in a local area network (LAN)      
...
#### **Bridge**: connects two or more network segments at the data link layer (Layer 2) to create a single, unified network.
...
#### **Switch**: device that connects multiple devices, and facilitates comms within a network.
...
#### **Router**: device that connects different local area networks (LANs), facilitates comms between networks.
